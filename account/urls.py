...
urlpatterns = [
    ...
    path('logout/', auth_views.LogoutView.as_view(next_page='/account/login/'), name='logout'),   # Once logged out, user will be redirected to the '/account/login/' page
    path('register/', views.account_register, name='register'),
    path('activate/<slug:uidb64>/<slug:token>/', views.account_activate, name='activate'), # We are capturing the user Id and the Token from the template
    # Reset password
    path('password_reset/', auth_views.PasswordResetView.as_view(template_name="account/user/password_reset_form.html", # This will reset the password
                                                                 success_url='password_reset_email_confirm',  # This will confirm the reset
